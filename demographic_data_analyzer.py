...
def calculate_demographic_data(print_data=True):
    ...
    # If there are no workers with min hours, avoid division by zero - use 0.0
    ...
    country_rich_percentage = (country_rich_counts / country_counts).fillna(0)
    highest_earning_country = country_rich_percentage.idxmax()
    highest_earning_country_percentage = round(country_rich_percentage.max() * 100, 1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]['occupation'].value_counts()
    top_IN_occupation = top_IN_occupation.idxmax() if not top_IN_occupation.empty else None
